...
Try running the code below (feel free to modify the values in the tibble). When you run the `predict()` function, you might get an error! This is likely because the API is not "open access" and requires that an API key be provided.
...
Instead, you will need to create an API key on the Connect server (click your name in the top right corner, select `API keys`), and save this key as an environment variable (e.g., `CONNECT_API_KEY`).
