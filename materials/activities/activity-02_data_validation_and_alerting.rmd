---
title: "activity-02_data_validation_and_alerting"
description: "A notebook that performs data validation and alerting using `pointblank` and `blastula`"
output:
  html_document:
    toc: yes
    toc_float: yes
...
- Read the `inspections_raw` data from the database
...
- How do you decide what data to filter out before validation versus what data to remove for not passing validation? Chat with your neighbor and share on Slido
...
This is our first first validation. It just confirms the overall size and format of the data frame. If the validation fails here, a warning is thrown and the validation will not proceed.
...
model_inputs <- inspections_processed |> 
  ...
  select(dba_name, license_number, facility_type, risk, address, city, zip, inspection_date, latitude, longitude, v_cumsum, v_cumsum_cs) |> 
  group_by(license_number) |> 
  arrange(desc(inspection_date)) |> 
  slice(1) |> 
  ungroup() |> 
