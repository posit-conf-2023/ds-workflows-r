...
```{r}
v <- vetiver_model(model = model_fit, model_name = "diamonds_price")
```
...
![](../images/vetiver_api_example.png){fig-align="center"}
...
```{r}
vetiver_deploy_rsconnect(board = connect_board, name = "your-name/diamonds_price")
```
...
model_recipe2 <- recipes::recipe(price ~ ., data = diamonds_train)

# Create another model type (decision_tree(mode = "regression"))
model_type2 <- parsnip::decision_tree(mode = "regression")

# Combine the recipe and model type into a new woriflow
model_wflow2 <- workflow(model_recipe2, model_type2)

# Fit the model to data
model_fit2 <- fit(model_wflow2, diamonds_train)

# Create a vetiver model (use the same name as before)
v2 <- vetiver_model(model = model_fit2, model_name = "your-name/diamonds_price")

# Pin the new model to same location
vetiver_pin_write(board = connect_board, vetiver_model = v2)

# View both versions of your model
pin_versions(board = connect_board, name = "your-name/diamonds_price")
```
...
augment(vetiver_pin_read(connect_board,
                         "your-name/diamonds_price",
                         version = "____"), # add in the correct version number of your model
        new_data = diamonds_new) |> 
  vetiver_compute_metrics(date_var = date_obs, 
                        period = "month", 
                        truth = price,  
